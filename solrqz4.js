...
function ssQzPocoChk(x) {
	let y = [
		{"pr": "newfoundlandlabrador", "pc": ["a"]},
		{"pr": "novascotia", "pc": ["b"]},
		{"pr": "princeedwardisland", "pc": ["c"]},
		{"pr": "newbrunswick", "pc": ["e"]},
		{"pr": "quebec", "pc": ["g", "h", "j"]},
		{"pr": "ontario", "pc": ["k", "l", "m", "n", "p"]},
		{"pr": "manitoba", "pc": ["r"]},
		{"pr": "saskatchewan", "pc": ["s"]},
		{"pr": "alberta", "pc": ["t"]},
		{"pr": "britishcolumbia", "pc": ["v"]},
		{"pr": ["nunavut", "northernterritories"], "pc": ["x"]},
		{"pr": "yukon", "pc": ["y"]}
	], z = "";
	y.forEach(a => {
		a.pc.forEach(b => {
...
function ssQzSlider(x, qz) {
	if(x === undefined || qz === undefined) {return}
	if(!x.hasOwnProperty("el")) {return}
	let y = x.el.getAttribute("data-ss-qz");
	// slides
	if(y == "slide" && qz.hasOwnProperty("slide")) {
		qz.slide.forEach((z, i) => {
			if(!z.hasOwnProperty("options") || 
				z.hasOwnProperty("options") && !z.options.hasOwnProperty("current")) {
				if(i === 0) {qz.slide[i].options.current = true}
				else {qz.slide[i].options.current = false}
			}
		})
	}
	// prev & next
